...
class NPC(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, animation_paths, speed=120, scale=0.5, direction=None):
        ...
        # Collision avoidance
        self.collision_rect = self.rect.inflate(-10, -10)  # Rectángulo más pequeño para colisiones
        self.collision_cooldown = 0

    def update_interaction_indicator(self):
        """Update the interaction indicator color based on NPC state."""
        self.interaction_indicator.fill((0, 0, 0, 0))  # Clear with transparent
        ...
        pygame.draw.circle(self.interaction_indicator, color, (8, 8), 8)
    ...
    def change_direction(self):
        ...
        # Update the image immediately to reflect the direction change
        if self.current_animation in self.animation_frames and len(self.animation_frames[self.current_animation]) > 0:
            if self.facing_right:
                self.image = self.animation_frames[self.current_animation][self.frame_index]['original']
            else:
    ...
    def check_collision(self, other_npcs):
        ...
        for npc in other_npcs:
            if npc != self and not npc.is_interacting:
                # Update the collision rectangle of the other NPC
                ...
                if self.collision_rect.colliderect(npc.collision_rect):
                    ...
                    if random.random() < 0.5:
                        ...
                        if overlap.width < overlap.height:
                            ...
                            # Vertical collision (less common in this case)
    ...
    def update(self, dt, screen_width, camera=None, other_npcs=None):
        ...
        if self.can_change_direction and random.random() < self.direction_change_chance:
            ...
            self.direction_change_cooldown = 1.0  # 1 second cooldown
        
        # Check for collisions with other NPCs
        if other_npcs:
            self.check_collision(other_npcs)
        
        # Update position if not interacting
        if self.direction.length() > 0:
            # Speed independent of framerate (pixels/second)
            move_x = self.direction.x * self.speed * dt
            
            # Update float position for precision
            self._float_pos.x += move_x
            
            # Update rect position
            self.rect.x = round(self._float_pos.x)
            
            # Check if the NPC is off-screen
            is_offscreen = False
            
            if camera:
                # Calculate the visible screen position
            ...
            if is_offscreen:
                ...
                self.time_offscreen = 0  # Reset counter if on-screen
    ...
    def draw_interaction_indicator(self, screen, camera):
        ...
        if self.can_interact:
            indicator_pos = camera.apply_point((self.rect.centerx, self.rect.top - 15))
            screen.blit(self.interaction_indicator, (indicator_pos[0] - 8, indicator_pos[1] - 8))
